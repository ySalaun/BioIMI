...
% ellipso�de : size of axis R and directions Vec
% I is the image of probabilities to be in the nodule
% c_pca is the center of the ellipso�de determined by the pca
...
for i=Int1
    for j=Int2
        for k=Int3
            ...
            if (d<=1) % si le voxel est dans l'ellipse test�e
...
for v1=1:size(x0_var,2)
    ...
    % faire varier les vecteurs tout en gardant l'orthogonalit� des axes
...
%     sol =A\b
%     if ( sum(sol<=Inf )==2 && efficiency~=-Inf ) %supprimer les cas NaN et double Inf de non r�solution
%         compte=compte+1;
%         if (sol(1)<Inf)
%             X(3,2)=sol(1);
%         end
...
%     end
    ...
    %test1 = sum(X(1,:).*X(2,:)) % v�rifer l'orthogonalit�
    %test2 = sum(X(1,:).*X(3,:))
    %test3 = sum(X(3,:).*X(2,:))
